...
export default function LoginScreen() {
  const { signIn, signInWithEmail, signUpWithEmail, signInAsGuest } = useAuth();
  ...
  const parseAuthError = (err: any) => {
    ...
    if (code === 'auth/unauthorized-domain') {
      return 'Google Sign-In domain restriction: This web domain is not added to Firebase Authorized Domains. Please use Email/Password login or click "Continue as Guest" below.';
    }
    if (code === 'auth/operation-not-allowed') {
      return 'Email/Password or Google auth is disabled in your Firebase console settings. Click "Continue as Guest" below to access TradeLog immediately.';
    }
    ...
    return err?.message || 'Authentication failed. Please try again or continue as guest.';
  };
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-[#0d1117] text-[#e6edf3] p-4">
      <div className="bg-[#161b22] p-8 rounded-xl border border-[#30363d] max-w-md w-full">
        ...
        {error && (
          <div className="mb-4 p-3 bg-[#f851491a] border border-[#f851494d] text-[#f85149] text-xs rounded text-left space-y-2">
            <p className="font-semibold">{error}</p>
            <button
              onClick={signInAsGuest}
              type="button"
              className="text-xs text-indigo-400 hover:underline font-bold block"
            >
              → Click here to skip login and Continue as Guest
            </button>
          </div>
        )}
        ...
        <button 
          ...
          disabled={loading}
          className="w-full py-3 bg-[#21262d] hover:bg-[#30363d] text-[#e6edf3] rounded-lg font-semibold text-sm border border-[#30363d] transition-colors flex items-center justify-center gap-2 mb-3 disabled:opacity-50"
        >
        ...
        <button
          onClick={signInAsGuest}
          type="button"
          className="w-full py-2.5 bg-indigo-600/10 hover:bg-indigo-600/20 text-indigo-400 rounded-lg font-bold text-xs border border-indigo-500/30 transition-colors mb-4"
        >
